...
#Phyloseq object
```{r}
setwd("/Users/karlavasco/Library/CloudStorage/OneDrive-MichiganStateUniversity/Manning_lab/Metabolomics_mastitis/tables/nonpolar/feature_tables")

library(readxl)
#read in otu table (smartchip normalized abundance)
otu_table <- read_excel("nonpolar_tables_mastitis_up.xlsx", sheet = "features")
...
taxonomy_df <- read_excel("nonpolar_tables_mastitis_up.xlsx", sheet = "taxonomy")
...
#read in metadata
metadata <- read_excel("nonpolar_tables_mastitis_up.xlsx", sheet = "metadata")
...
write.csv(physeq_df,file = "nonpolar_physeq_relab.csv")
```

```{r}
physeq <- physeq_nocontrols_relab
physeq_Component = tax_glom(physeq_nocontrols_relab, "Component")
```
...
imp.sort_components <- merge(imp.sort_components,taxonomy_df, by="Cluster_ID") %>% 
  dplyr::select(Component,`Day..1`,`Week.1`,`Week.9`,MeanDecreaseAccuracy,MeanDecreaseGini) %>% arrange(desc(MeanDecreaseAccuracy))
...
```
```{r}
setwd("/Users/karlavasco/Library/CloudStorage/OneDrive-MichiganStateUniversity/Manning_lab/Metabolomics_mastitis/tables/nonpolar/differential_analyzes")

library(xlsx)
write.xlsx(imp.sort_clusters, file="nonpolar_time_diff.xlsx", sheetName="randomForest_clust", row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_time_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
```
#Exporting physeq as dataframe
```{r}
physeq_component_df <- psmelt(physeq_Component) 
physeq_cluster_df <- psmelt(physeq) 
```

```{r}
imp.50_component <- imp.sort_components[1:50, ]
imp.50_clusters <- imp.sort_clusters[1:50, ]
imp.50_clusters <- imp.50_clusters %>% mutate(predictors = gsub("Cluster.|\\.\\d+[A-Za-z]+", "Cluster ", predictors)) 
ps.clust.randomforest <- prune_taxa(imp.50_clusters$predictors, physeq)
ps.comp.randomforest <- prune_taxa(imp.50_component$Component, physeq_Component)
```

```{r}
setwd("/Users/karlavasco/Library/CloudStorage/OneDrive-MichiganStateUniversity/Manning_lab/Metabolomics_mastitis/tables/nonpolar/differential_analyzes")

physeq_component_df <- psmelt(ps.comp.randomforest) 
physeq_cluster_df <- psmelt(ps.clust.randomforest) 

write.csv(physeq_cluster_df,file = "nonpolar_physeq_clusterRF_relab.csv")

write.csv(physeq_component_df,file = "nonpolar_physeq_componentRF_relab.csv")
```
...
stat.test2 <- shannon %>%  filter(Time_tx %in% c("Week 1","Week 9")) %>%
  group_by(Time_tx) %>%
  wilcox_test(value ~ Treatment, alternative = "less") %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj") %>%
  add_xy_position(x = "Time_tx", dodge = 0.8)
stat.test2
...
shannon_nonpolar = ggboxplot(shannon,x = "Time_tx", y = "value", color = "Treatment", palette = "aaas", fill = "Treatment", add = c("jitter"), notch = F, outlier.shape = NA, facet.by = "variable", panel.labs = list(variable=c("Nonpolar","Ob"))) +
...
#  ylim(3,9)+
  ...
  annotate("text", x = 2, y = 7.1, 
           label = expression(paste("Friedman test, ", paste(italic('p'))," = 0.0021"))) +
  geom_signif(aes(y = value, x = Time_tx),
              comparisons = list(c("Day -1","Week 1"),c("Week 1","Week 9"),c("Day -1","Week 9")), 
              test = "wilcox.test", test.args = c(alternative = "greater"),  y_position = c(6.5,6.6,6.8), 
...
```{r}
library(rstatix)
Observed <- alpha %>% filter(variable%in%"Observed")
#Calculating p-value for the variation over time
...
observed_nonpolar = ggboxplot(Observed,x = "Time_tx", y = "value", color = "Treatment", palette = "aaas", fill = "Treatment", add = c("jitter"), notch = F, outlier.shape = NA, facet.by = "variable", panel.labs = list(variable=c("Nonpolar","Nonpolar"))) +
  labs(x = "Time to treatment", y = "Observed") + 
  scale_fill_aaas(alpha = 0.5) +
#  ylim(3,9)+
  stat_pvalue_manual(stat.test,  label = "p", tip.length = 0) +
  annotate("text", x = 2, y = 10.4E3, 
           label = expression(paste("Friedman test, ", paste(italic('p'))," = 0.00025"))) +
  geom_signif(aes(y = value, x = Time_tx),
              comparisons = list(c("Day -1","Week 1"),c("Day -1","Week 9"),c("Week 9","Week 1")), 
              test = "wilcox.test", test.args = c(alternative="greater"), y_position = c(10E3,10.2E3,10.1E3), 
              color = "gray35", tip_length = 0.02)  +
  theme(axis.title.x = element_blank())
observed_nonpolar
```
...
bray_nonpolar <- plotbeta(
  ...
  annotate("text", x = 0.1, y = 0.18, label = expression(paste("PERMANOVA, ",F ,"= 6.59, ",paste(italic('p')),"=0.001")), colour = "black", size = 4) + ggtitle("Nonpolar")
  theme_bw()
bray_nonpolar
```
...
```
```{r}
setwd("/Users/karlavasco/Documents/figures")
ggsave(plot=diversity_plots,"non_polar_diversity_plots.png", height = 10,width = 10)
```
...
row.names(ancom_signif_names) <- rownames(ancom_signif_list$beta) 
ancom_signif_names <- tibble::rownames_to_column(ancom_signif_names, "Cluster_ID")
ancom_components <- merge(taxonomy_df,ancom_signif_names,by.x = "Cluster_ID") %>% select(Component,17:22)
```

```{r}
setwd("/Users/karlavasco/Documents/nonpolar")

library(xlsx)
write.xlsx(as.data.frame(lefse_sigtab), file="nonpolar_day1_treatment_diff.xlsx", sheetName="lefse", row.names=FALSE)
write.xlsx(imp.sort_clusters, file="nonpolar_day1_treatment_diff.xlsx", sheetName="randomForest_clust", append=TRUE, row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_day1_treatment_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
write.xlsx(ancom_clusters, file="nonpolar_day1_treatment_diff.xlsx", sheetName="ancom_clusters", append=TRUE, row.names=FALSE)
write.xlsx(ancom_components, file="nonpolar_day1_treatment_diff.xlsx", sheetName="ancom_components", append=TRUE, row.names=FALSE)
```
###MaAsLin2
Clusters
```{r}
...
library(xlsx)
write.xlsx(as.data.frame(lefse_sigtab), file="nonpolar_week1_treatment_diff.xlsx", sheetName="lefse", row.names=FALSE)
write.xlsx(imp.sort_clusters, file="nonpolar_week1_treatment_diff.xlsx", sheetName="randomForest_clust", append=TRUE, row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_week1_treatment_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
write.xlsx(ancom_clusters, file="nonpolar_week1_treatment_diff.xlsx", sheetName="ancom_clusters", append=TRUE, row.names=FALSE)
write.xlsx(ancom_components, file="nonpolar_week1_treatment_diff.xlsx", sheetName="ancom_components", append=TRUE, row.names=FALSE)
```
###MaAsLin2
Clusters
```{r}
...
library(xlsx)
write.xlsx(as.data.frame(lefse_sigtab), file="nonpolar_week9_treatment_diff.xlsx", sheetName="lefse", row.names=FALSE)
write.xlsx(imp.sort_clusters, file="nonpolar_week9_treatment_diff.xlsx", sheetName="randomForest_clust", append=TRUE, row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_week9_treatment_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
write.xlsx(ancom_clusters, file="nonpolar_week9_treatment_diff.xlsx", sheetName="ancom_clusters", append=TRUE, row.names=FALSE)
write.xlsx(ancom_components, file="nonpolar_week9_treatment_diff.xlsx", sheetName="ancom_components", append=TRUE, row.names=FALSE)
```
...
#library(xlsx)
write.xlsx(as.data.frame(lefse_sigtab), file="nonpolar_d1_w1_Time_tx_diff.xlsx", sheetName="lefse", row.names=FALSE)
write.xlsx(imp.sort_clusters, file="nonpolar_d1_w1_Time_tx_diff.xlsx", sheetName="randomForest_clust", append=TRUE, row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_d1_w1_Time_tx_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
write.xlsx(ancom_clusters, file="nonpolar_d1_w1_Time_tx_diff.xlsx", sheetName="ancom_clusters", append=TRUE, row.names=FALSE)
write.xlsx(ancom_components, file="nonpolar_d1_w1_Time_tx_diff.xlsx", sheetName="ancom_components", append=TRUE, row.names=FALSE)
```
...
library(xlsx)
write.xlsx(as.data.frame(lefse_sigtab), file="nonpolar_d1_week9_Time_tx_diff.xlsx", sheetName="lefse", row.names=FALSE)
write.xlsx(imp.sort_clusters, file="nonpolar_d1_week9_Time_tx_diff.xlsx", sheetName="randomForest_clust", append=TRUE, row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_d1_week9_Time_tx_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
write.xlsx(ancom_clusters, file="nonpolar_d1_week9_Time_tx_diff.xlsx", sheetName="ancom_clusters", append=TRUE, row.names=FALSE)
write.xlsx(ancom_components, file="/Users/karlavasco/Documents/nonpolar/nonpolar_d1_week9_Time_tx_diff.xlsx", sheetName="ancom_components", append=TRUE, row.names=FALSE)
```
...
library(xlsx)
write.xlsx(as.data.frame(lefse_sigtab), file="nonpolar_w1_week9_Time_tx_diff.xlsx", sheetName="lefse", row.names=FALSE)
write.xlsx(imp.sort_clusters, file="nonpolar_w1_week9_Time_tx_diff.xlsx", sheetName="randomForest_clust", append=TRUE, row.names=FALSE)
write.xlsx(imp.sort_components, file="nonpolar_w1_week9_Time_tx_diff.xlsx", sheetName="randomForest_compo", append=TRUE, row.names=FALSE)
write.xlsx(ancom_clusters, file="nonpolar_w1_week9_Time_tx_diff.xlsx", sheetName="ancom_clusters", append=TRUE, row.names=FALSE)
write.xlsx(ancom_components, file="nonpolar_w1_week9_Time_tx_diff.xlsx", sheetName="ancom_components", append=TRUE, row.names=FALSE)
```
